/**
 * News Animations JavaScript
 * Thêm các hiệu ứng và animation cho trang tin tức
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Thêm class news-item cho các tin tức để áp dụng animation
    const newsItems = document.querySelectorAll('.col-md-6.col-lg-6.mb-4');
    ...
    // Thêm badge "Mới" cho tin tức mới (3 tin tức đầu tiên)
    const addNewsBadges = () => {
        const newsCards = document.querySelectorAll('.news-card');
        if (newsCards.length > 0) {
            // Thêm badge "Mới" cho 3 tin tức đầu tiên
            for (let i = 0; i < Math.min(3, newsCards.length); i++) {
                const badgeNew = document.createElement('div');
                badgeNew.className = 'news-badge badge-new';
                badgeNew.innerHTML = '<i class="fas fa-bolt me-1"></i> Mới';
                
                // Kiểm tra xem đã có badge chưa
                if (!newsCards[i].querySelector('.news-badge')) {
                    newsCards[i].appendChild(badgeNew);
                }
            }
            
            // Thêm badge "Nổi bật" cho tin tức thứ 4, 5 (nếu có)
            for (let i = 3; i < Math.min(5, newsCards.length); i++) {
                const badgeFeatured = document.createElement('div');
                badgeFeatured.className = 'news-badge badge-featured';
                badgeFeatured.innerHTML = '<i class="fas fa-star me-1"></i> Nổi bật';
                
                // Kiểm tra xem đã có badge chưa
                if (!newsCards[i].querySelector('.news-badge')) {
                    newsCards[i].appendChild(badgeFeatured);
                }
            }
        }
    };
    
    // Thêm class cho icon trong nút đọc tiếp
    const addButtonIconClass = () => {
        const readMoreButtons = document.querySelectorAll('.news-card .btn-outline-primary .fa-arrow-right');
        readMoreButtons.forEach(icon => {
    ...
    function showSkeletonLoading() {
        ...
        if (newsContainer) {
            ...
            newsContainer.className = 'row'; // Đảm bảo class row được giữ lại
            
            // Tạo 4 skeleton cards
            for (let i = 0; i < 4; i++) {
                const skeletonCol = document.createElement('div');
                skeletonCol.className = 'col-md-6 col-lg-6 mb-4';
                
                const skeletonCard = document.createElement('div');
                skeletonCard.className = 'skeleton-news';
                ...
                newsContainer.appendChild(skeletonCol);
            }
        }
    }
            ...
            .then(data => {
                ...
                setTimeout(() => {
                    ...
                    if (newsContainer) {
                        ...
                        // Thêm class news-item cho các tin tức mới
                        const newNewsItems = newsContainer.querySelectorAll('.col-md-6.col-lg-6.mb-4');
                        newNewsItems.forEach(item => {
                            item.classList.add('news-item');
                        });
                        
                        // Thêm các hiệu ứng cho tin tức mới
                        addNewsBadges();
                        addButtonIconClass();
                        
                        // Thêm hiệu ứng xuất hiện cho các mục tin tức
                    ...
                    if (countDisplay && data.count) {
                        countDisplay.textContent = `Hiển thị ${data.count.visible} / ${data.count.total} bài viết`;
                    }
    ...
    // Thêm hiệu ứng cho các thẻ tin tức khi tải trang
    ...
    // Thêm các hiệu ứng cho tin tức
    addNewsBadges();
    addButtonIconClass();
    
    // Gọi hàm animateNewsCards khi trang được tải
    animateNewsCards();
    ...
    // Thêm hiệu ứng cho trang chi tiết tin tức
    if (document.getElementById('blogPostContent')) {
        // Tạo mục lục tự động
        const headings = document.querySelectorAll('#blogPostContent h2, #blogPostContent h3');
        ...
        if (headings.length > 0 && tocList) {
            headings.forEach((heading, index) => {
                ...
                // Thêm class dựa vào loại heading
                ...
                // Thêm hiệu ứng scroll mượt mà khi click vào mục lục
            ...
            // Ẩn mục lục nếu không có heading
